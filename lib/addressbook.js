...
module.exports = function (opts) {
    ...
    case topics.msg:
      ...
      parallel(value.pubkeys.map(function (key) {
        ...
        value.pubkeys.forEach(function (key) {
          value[getPubKeyKey(key)] =
          value[getFingerprintKey(key)] = value.link
        })
  ...
  function byPubKey (pubKey, cb) {
    utils.firstFromIndex(ixf.index, getPubKeyKey(pubKey), function (err, val) {
      if (err) return cb(err)
  ...
  function byFingerprint (fingerprint, cb) {
    utils.firstFromIndex(ixf.index, getFingerprintKey(fingerprint), function (err, val) {
      if (err) return cb(err)
...
function getPubKeyKey (key) {
  return 'p:' + pubKeyToString(key)
}

function getFingerprintKey (key) {
  'f:' + key.fingerprint
}

function pubKeyToString (key) {
  return utils.hex(key.pub)
}
