...
module.exports = function createAddressBook (opts) {
  ...
  // const byCurHash = subdown(db, 'm')
  // const byRootHash = subdown(db, 'r')
  const indexes = subdown(db, 's')
  ...
  function worker (change, cb) {
    ...
    const batch = []
    const opType = change.type // 'put' or 'del'
    const rootHash = val[ROOT_HASH]
    const curHash = val[CUR_HASH]
    keeper.getOne(curHash).then(identity => {
  ...
  function createReadStream (opts) {
    opts = opts || {}
    const raw = byCurHash.createReadStream()
