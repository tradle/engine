...
function indexer (getBatch, getIndexKey, getIndexMap, getIndexedProps) {
  return function (db, processor, opts) {
    opts = opts || { valueEncoding: utils.clone(utils.defaultValueEncoding) }
    const main = subdown(db, 'm', opts)
    const index = subdown(db, 'x', opts)

    utils.live(main, processor)
    utils.live(index, processor)
    ...
    function createIndexStream (prop, value, opts) {
      ...
      const source = opts.live === false ? db : db.live
      let stream
      if (!db.isOpen()) {
    ...
    function getRangeOpts (db, prop, value) {
      const opts = {}
      const prefix = db.db.prefix
      opts.gt = prefix + prop + SEPARATOR
      if (value != null) {
    ...
    return {
      batchForChange: function (oldState, newState) {
        const batch = getBatch(oldState, newState, bOpts)
