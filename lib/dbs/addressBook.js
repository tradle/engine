...
const addressBookOpts = typeforce.object({
...
module.exports = function createAddressBook (opts) {
  ...
  complex.forEach(prop => {
    indexes[prop] = indexedDB.by(prop, function reducer (state, cb) {
      let newPubkeys = state.object.pubkeys.filter(key => !key.importedFrom)
      return newPubkeys.map(key => {
  ...
  function updateCache (identityInfo, remove) {
    ...
    // }
    let newPubkeys = identityInfo.object.pubkeys.filter(key => !key.importedFrom)
