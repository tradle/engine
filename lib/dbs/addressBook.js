...
module.exports = function createAddressBook (opts) {
  ...
  function findOneByPropInDB (propertyName, value, cb) {
    indexes[propertyName].findOne({ eq: value, keys: false }, function (err, result) {
      if (err) {
        if (err.notFound) {
          cb(new Errors.UnknownIdentity({
            query: JSON.stringify({ propertyName, value })
          }))
        } else {
          cb(err)
        }
