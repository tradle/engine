...
module.exports = function createSealsDB (opts) {
  ...
  const indexed = indexers.seal(db, processor)
  ...
  emitter.get = function (uid, cb) {
    indexed.main.get(uid, cb)
  }

  emitter.search = indexed.search.bind(indexed)
  emitter.first = indexed.first.bind(indexed)

  const streams = emitter.streams = {}
  streams.pending = function (opts) {
    return indexed.createIndexStream('status', SealStatus.pending, opts)
  }

  streams.sealed = function (opts) {
    return indexed.createIndexStream('status', SealStatus.sealed, opts)
  }
