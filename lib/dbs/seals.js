...
module.exports = function createSealsDB (opts) {
  ...
  const index = indexers.seal(main, indexDB)
  ...
  function worker (change, cb) {
    ...
    index.search('link', link, { live: false }, function (err, seals) {
      if (seals.length > 1) {
  ...
  emitter.get = function (uid, cb) {
    main.get(uid, cb)
  }

  emitter.search = index.search.bind(index)
  emitter.first = index.first.bind(index)

  const streams = emitter.streams = {}
  streams.pending = function (opts) {
    return index.createIndexStream('status', SealStatus.pending, opts)
  }

  streams.sealed = function (opts) {
    return index.createIndexStream('status', SealStatus.sealed, opts)
  }
