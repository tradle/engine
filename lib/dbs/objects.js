...
function createObjectDB (opts) {
  ...
  function list (type, cb) {
    ...
    const stream = type
      ? indexes.type.createReadStream(type)
      : indexes.latest.createReadStream()
  ...
  return utils.extend(emitter, {
    ...
    unsent: function (opts, cb) {
      return collect(indexes.sendstatus.createReadStream(SendStatus.pending), cb)
    },
    unsealed: function (opts, cb) {
      return collect(indexes.sealstatus.createReadStream(SealStatus.pending), cb)
    },
