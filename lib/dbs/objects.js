...
function createObjectDB (opts) {
  ...
  function byType (type, cb) {
    return collect(indexes.type.createReadStream({ eq: type, keys: false }), cb)
  }
  ...
  return utils.extend(emitter, {
    ...
    get: function (link, getBody, cb) {
      ...
      indexedDB.get(link, function (err, info) {
    ...
    sealed: function (minConfirmations, cb) {
      if (typeof minConfirmations === 'function') {
        cb = minConfirmations
        minConfirmations = 0
      }

      return indexes.confirmations.createReadStream({ gte: minConfirmations })
    },
    // createReadStream: createReadStream,
    list: function (cb) {
      return collect(indexes.latest.createReadStream({ keys: false }), cb)
    },
    messages: function (cb) {
      return byType(MESSAGE_TYPE, cb)
    },
