...
function createObjectDB (opts) {
  ...
  function from (permalink, cb) {
    return indexes.from.createReadStream({ eq: permalink, keys: false })
  }

  function to (permalink, cb) {
    return indexes.to.createReadStream({ eq: permalink, keys: false })
  }
  ...
  return utils.extend(emitter, {
    ...
    from: from,
    to: to,
    conversation: function (permalinkFrom, permalinkTo, cb) {
      return pump(
        from(permalinkFrom),
