...
const COUNTER = constants.COUNTER
...
function createObjectDB (opts) {
  ...
  const myDebug = utils.subdebugger(debug, name || me.permalink.slice(0, 6))
  ...
  return utils.extend(emitter, {
    ...
    nextMessageID: function (opts, cb) {
      typeforce({
        with: typeforce.String,
      }, opts)
