...
function createObjectDB (opts) {
  ...
  indexes.messagesWithObject = indexedDB.by('messagesWithObject', function (state) {
    if (state.type === MESSAGE_TYPE) {
      return state.objectpermalink + sep + state.objectlink + sep + state.permalink
    }
  })
  ...
  return utils.extend(emitter, {
    ...
    messagesWithObject: function (opts, cb) {
      typeforce({
      ...
      return collect(indexes.messagesWithObject.createReadStream({ eq: query, keys: false }), cb)
    }
