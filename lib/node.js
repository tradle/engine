...
function Tradle (opts) {
  ...
  typeforce({
    ...
    name: typeforce.maybe(types.String)
  }, opts)
  ...
  // reemit(this.objects, this, ['sent', 'message', 'wroteseal', 'readseal'])
...
proto.saveObject = function saveObject (opts, cb) {
  ...
  async.series([
    ...
    self._debug('save queued', err || '')
    cb(err)
  })

  function checkExists (done) {
    self._debug('save 1. check existence')
  ...
  function createAction (done) {
    self._debug('save 3. create action')
...
proto.send = function send (opts, cb) {
  ...
  async.waterfall([
    ...
    function lookupRecipient (done) {
      self._debug('send 3. lookup recipient')
...
proto._trackchain = function _trackchain (chaintracker) {
  ...
  this.sealwatch = createSealWatcher({
