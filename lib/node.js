...
proto.sign = function sign (opts, cb) {
  ...
  protocol.sign({
    ...
    // result.object[SIGNEE] = utils.signee(self)
...
proto.addContact = function addContact (identity, cb) {
  ...
  function checkExists (done) {
    ...
    async.find(indexes, function (indexVal, taskCB) {
      self.addressBook.lookupIdentity(indexVal, function (err, _match) {
        match = match || _match
        taskCB(err, !_match)
      })
    }, function (err) {
      ...
      console.log(match.object, identity)
      if (deepEqual(match.object, identity)) return done()
...
proto.forget = function forget (link, cb) {
  const self = this
  const stream = this.conversation(link)
