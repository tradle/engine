...
proto.addContactIdentity = function addContact (identity, cb) {
  ...
  async.waterfall([
    ...
    function log (done) {
      ...
      self.changes.append(entry, done)
    }
  ], cb)
}
...
proto.saveObject = function (opts, cb) {
  ...
  async.series([
    function objectExists (done) {
      self.objectDB.get(uid, function (err) {
...
proto.send = function (opts, cb) {
  ...
  async.waterfall([
    ...
    function createMessage (identityInfo, done) {
      // rLink = utils.identityLink(identityInfo)
      protocol.message({
        object: object,
  ...
  async.each(wrappers, function validate (wrapper, done) {
    ...
    utils.saveToKeeper(self.keeper, wrappers, function (err) {
      ...
      async.each(wrappers, function log (done) {
        self.changes.append({
          topic: topics.newobj,
          // same as utils.uid(wrapper.object), but potentially faster
          uid: utils.uid(wrapper),
          link: wrapper[LINK],
          permalink: wrapper[PERMALINK],
          prevlink: wrapper[PREVLINK],
          // recipient: self.permalink,
          author: wrapper.author[PERMALINK],
          type: object[TYPE]
        }, done)
      }, function (err) {
        if (err) return cb(err)
...
proto.seal = function (opts, cb) {
  ...
  this.objectDB.hasSealWithID(sealID, function (err, exists) {
    ...
    const sealPrev = object[PREVLINK] && protocol.sealPrevPubKey({
      object: object,
      basePubKey: basePubKey
    })

    self.changes.append({
      topic: topics.queueseal,
      sealID: sealID,
      link: link,
      sealThis: sealThis,
      sealPrev: sealPrev,
