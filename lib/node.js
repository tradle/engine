...
function Tradle (opts) {
  ...
  ;['sent', 'message', 'wroteseal', 'readseal'].forEach(event => {
    this.objects.on(event, state => {
      // if (this.name === 'bob' && event === 'readseal') console.log(state)
      // if (event === 'readseal' && !state.link) {
      //   return this.emit('newversionexists', state.prevLink)
      // }
  ...
  // sequentially
  // utils.lockify(this, ['createObject'])
  this._saving = {}
}
