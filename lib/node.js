 ...
 * @param {Network} opts.network          see github:tradle/bitcoin-adapter, github:tradle/ethereum-adapter
...
// Tradle.prototype._setIdentity = function (opts, cb) {
  ...
  this.chainPubKey = utils.chainPubKey(this.identity, this.network.blockchain)
...
Tradle.prototype._queueSend = function _queueSend (opts, cb) {
  ...
  function createMessage (results, done) {
    ...
    const required = { [TYPE]: MESSAGE_TYPE, object, recipientPubKey, time }
    ...
    self.keeper.get(link, function (err, object) {
      ...
      if (prevLink) {
        sealPrevPubKey = protocol.sealPrevPubKey({
          basePubKey: basePubKey,
          prevLink: utils.linkToBuf(prevLink)
        })
...
Tradle.prototype.watchNextVersion = function (opts, cb) {
  ...
  const { network } = this
  const address = utils.sealPrevAddress({ network, basePubKey, link })
  this._debug('watching (next version) seal at address:', address)
  this._watch({
...
Tradle.prototype.watchSeal = function (opts, cb) {
  ...
  this._debug('watching seal at address:', address)
  this._watch({
    address,
    basePubKey,
    link,
...
Tradle.prototype.destroy = function destroy (cb) {
  ...
  ;[this.blockchain, this.transactor].forEach(stoppable => {
