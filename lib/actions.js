...
module.exports = function (opts) {
  ...
  function createObject (wrapper, cb) {
    typeforce({
      ...
      objectlink: typeforce.maybe(typeforce.String),
      objectpermalink: typeforce.maybe(typeforce.String),
    }, wrapper)

    const entry = utils.pick(wrapper, 'link', 'permalink', 'prevLink', 'author', 'recipient', 'objectlink', 'objectpermalink')
    ...
    if (entry.type === MESSAGE_TYPE) {
      ...
      if (!entry.objectlink || !entry.objectpermalink) {
        const links = utils.getLinks({ object: wrapper.object.object })
        entry.objectlink = links.link
        entry.objectpermalink = links.permalink
      }
    }
