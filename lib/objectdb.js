...
const indexers = require('./indexer').indexers
...
module.exports = function (opts) {
  ...
  const main = liveDB(db, 'm', dbOpts)
  const objIndex = liveDB(db, 'x', dbOpts)
  ...
  function processSealChange (change, cb) {
    ...
    changeVal.uid = utils.sealUID(changeVal)

    main.get(changeVal.uid, function (err, state) {
  ...
  function processObjectChange (change, cb) {
    ...
    main.get(lookupUID, function (err, state) {
      const newState = reducer(state, changeVal)
      const batch = getObjectBatch(state, newState)
      saveAndEmit(batch, newState, changeVal, cb)
    })
  }
  ...
  streams.type = function (type, opts) {
    return createIndexStream(objIndex, 'type', type, opts)
  }

  streams.unsent = function (opts) {
    return createIndexStream(objIndex, 'sendstatus', SendStatus.pending, opts)
  }
