...
module.exports = function sealwatch (opts) {
  ...
  function processTx (txInfo, cb) {
    ...
    async.find(toAddrs, function iterator (address, done) {
      ...
      if (watch.txId && watch.confirmations === txInfo.confirmations) {
        return cb()
      }

      // if (results.length > 1) {
      //   debug('transaction matches multiple watches: ', txInfo.txId, results)
      // }

      const knownConfirmations = watch.confirmations
      if (txInfo.confirmations <= knownConfirmations) return done()
