...
function sealer (opts) {
  ...
  function writeSeal (state, cb) {
    let data
    let to = [getPrimarySealAddress(state)]
    const secondary = getSecondarySealAddress(state)
    if (secondary) {
      if (transactor.multipleRecipientsAllowed === false) {
        data = secondary
        myDebug(`embedding new version seal ${state.sealAddress} in transaction data`)
        // console.log(`embedding new version seal ${state.sealAddress} in transaction data`)
      } else {
        to.push(secondary)
      }
    }

    // console.log(`${opts.name} sealing ${state.link} to [${to.join(', ')}]`)
  ...
  function start () {
    ...
    pump(
      ...
      through.obj(function (data, enc, cb) {
        ...
        myDebug(`sealed ${data.input.link} to address ${getPrimarySealAddress(data.input)} with tx ${data.output.txId}`)
        // console.log(opts.name, `sealed ${data.input.link} to address ${getPrimarySealAddress(data.input)} with tx ${data.output.txId}`)
        cb()
      })
    )
  ...
  function getPrimarySealAddress ({ sealPrevAddress, sealAddress }) {
    return sealPrevAddress || sealAddress
  }

  function getSecondarySealAddress ({ sealPrevAddress, sealAddress }) {
    return sealPrevAddress ? sealAddress : null
  }
}
