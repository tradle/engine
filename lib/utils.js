...
exports.watchUID = function watchUID (opts) {
  ...
  return `watch:${opts.address}:${opts.link}`
}
...
exports.sealUID = function sealUID (opts) {
  ...
  const address = opts.sealAddress || utils.pubKeyToAddress(opts.sealPubKey, opts.networkName)
  if (!address) throw Error('unable to derive address')

  return `seal:${opts.networkName}:${address}:${opts.link}`
}
