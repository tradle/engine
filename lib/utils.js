...
exports.prefixKey = function (prefix, key /*, forDB*/) {
  var sep = '!'
...
exports.encodeBatch = function encodeBatch (rawBatch /*, forDB*/) {
...
exports.encodeRow = function encodeRow (row /*, forDB*/) {
  const encoded = {
    type: row.type,
 ...
 */
exports.notReadyStream = function (fn) {
...
exports.addPrev = function addPrev (node, wrapper, cb) {
...
exports.lookupAuthor = function lookupAuthor (node, wrapper, cb) {
  ...
  if (author) {
    ...
    if (author.link) {
      return node.addressBook.lookupIdentity(author.link, cb)
    }
  }

  const sigPubKey = utils.getSigPubKey(wrapper.object)
  node.addressBook.lookupIdentity(utils.pubKeyString(sigPubKey), cb)
}
...
exports.loadBG = function loadBG (node, wrapper, cb) {
  ...
  const tasks = [
    utils.addAuthor.bind(utils, node, wrapper),
    utils.addPrev.bind(utils, node, wrapper)
  ]

  async.parallel(tasks, cb)
}

exports.getSigPubKey = function (object) {
