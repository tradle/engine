...
exports.chainKey = function (keys, chain) {
  return utils.find(keys, key => isChainKey(key, chain), isPrivateKey)
}

// exports.chainKeys = keys => keys.filter(isChainKey)

exports.chainPubKey = function chainPubKey (identity, chain) {
  const key = utils.find(identity.pubkeys, key => isChainKey(key, chain))
  return key && utils.toECKeyObj(key)
}
...
exports.sealAddress = function sealAddress ({ network, basePubKey, link }) {
...
exports.sealPrevAddress = function sealPrevAddress ({ network, basePubKey, link }) {
...
exports.defaultKeySet = function defaultKeySet (networks) {
  ...
  const networkKeys = Object.keys(networks).map(chainName => {
    const networkName = networks[chainName] || DEFAULT_NETWORK[chainName]
    return [
...
exports.serializeMessage = function (msg) {
  ...
  for (var p in msg) {
    ...
    case 'seal':
      copy.seal = {
        network: +utils.keyByValue(SchemaNetworkToName, val.network),
...
function isChainKey (key, chain) {
  return getKeyProp(key, 'type') === chain
}
...
function loadNKeyImplementations () {
  ...
  try {
    impls.ethereum = require('nkey-ethereum')
  } catch (err) {}
