...
//     typeof obj === 'string' ? Buffer.from(obj, enc) :
//       Buffer.from(JSON.stringify(obj), enc)
// }
...
//       break
//     case 'object':
//       copy[p] = Buffer.from(stringify(val))
//       break
//     case SIG:
...
//         link: Buffer.from(val.link, 'hex'),
//         headerHash: Buffer.from(val.headerHash, 'hex'),
//       }
...
//     copy.other = Buffer.from(JSON.stringify(other))
//   }
...
//   const formatted = {
//     [SIG]: Buffer.from(identity[SIG], 'base64'),
//     pubkeys: identity.pubkeys.map(utils.toCompactPubKey)
//   }

//   if (identity[PERMALINK]) formatted[PERMALINK] = Buffer.from(identity[PERMALINK], 'hex')
//   if (identity[PREVLINK]) formatted[PREVLINK] = Buffer.from(identity[PREVLINK], 'hex')
...
//     pub: Buffer.from(key.pub, key.type === 'dsa' ? 'base64' : 'hex')
//   }
...
//   if (typeof object === 'string') object = Buffer.from(object)
