...
exports.uid = function (obj, link) {
  const curHash = utils.hexLink(link || obj)
  const rootHash = obj[PERMALINK] || curHash
  return curHash + ':' + rootHash
}

exports.prevUID = function (obj) {
  if (!obj[PREVLINK]) throw new Error(`missing ${PREVLINK}`)

  return utils.uid(obj, obj[PREVLINK])
}
...
exports.identityBatch = function (identity) {
  ...
  const rootHash = identity[PERMALINK] || curHash
...
exports.addLinks = function addLinks (wrapper) {
  ...
  if (!wrapper.link) wrapper.link = utils.hexLink(wrapper.object)
  if (!wrapper.prevlink) wrapper.prevlink = wrapper.object[PREVLINK]
  if (!wrapper.permalink) wrapper.permalink = wrapper.object[PERMALINK] || wrapper[LINK]
...
exports.addPrev = function addPrev (wrapper, db, cb)
  ...
  if (!wrapper.object[PREV]) return cb()

  const uid = utils.prevUID(wrapper.object)
...
exports.identitylink = function identityLink (identityInfo) {
  typeforce({
    [PERMALINK]: typeforce.String
  }, identityInfo)

  return utils.linkToBuf(identityInfo[PERMALINK])
}

exports.values = function (obj) {
  return Object.keys.map(key => obj[key])
}
