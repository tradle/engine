...
utils.defaultKeySet = function defaultKeySet (opts) {
  ...
  return utils.requiredKeys.map(k => {
    if (k.type === 'bitcoin' && !k.networkName) {
      k = utils.clone(k)
      k.networkName = 'testnet'
    }
...
utils.serializeMessage = function (msg) {
  ...
  for (var p in msg) {
    ...
    case 'recipientPubKey':
      copy.recipientPubKey = msg.recipientPubKey
      break
    case 'object':
      copy.object = new Buffer(stringify(msg.object))
      break
    case SIG:
      copy[SIG] = protocol.utils.parseSig(msg[SIG])
      break
    case PREV_TO_SENDER:
      copy[PREV_TO_SENDER] = new Buffer(msg[PREV_TO_SENDER], 'hex')
      break
    default:
      other[p] = msg[p]
      break
...
utils.unserializeMessage = function (msg) {
  ...
  for (var p in msg) {
    if (msg[p] == null) delete msg[p]
  }

  msg.object = JSON.parse(msg.object)
  msg[TYPE] = constants.MESSAGE_TYPE
  msg[SIG] = protocol.utils.sigToString(protocol.utils.encodeSig(msg[SIG]))
